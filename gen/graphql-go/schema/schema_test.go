...
func TestParse(t *testing.T) {
	for _, test := range []struct {
		...
		validateSchema        func(s *Schema) error
	}{
		{
			...
			validateSchema: func(s *Schema) error {
				...
				typ, ok := s.Types[typeName].(*Interface)
				if !ok {
		...
		},
		{
			...
			validateSchema: func(s *Schema) error {
				const typeName = "Type"
				typ, ok := s.Types[typeName].(*Object)
				if !ok {
...
Second line of the description.
		...
		},
		{
			...
			validateSchema: func(s *Schema) error {
				...
				typm, ok := s.Types["Mutation"].(*Object)
				if !ok {
		...
		},
		{
			...
			validateSchema: func(s *Schema) error {
				...
				ifc, ok := s.Types["Named"].(*Interface)
				if !ok {
		...
		},
		{
			...
			`,
			validateSchema: func(s *Schema) error {
				typ, ok := s.Types["Currencies"].(*Enum)
				if !ok {
		...
		},
		{
			...
			`,
			validateSchema: func(s *Schema) error {
				typ, ok := s.Types["Product"].(*InputObject)
				if !ok {
					return fmt.Errorf("type %q not found", "Product")
				}
				if len(typ.Values) != 4 {
		...
		},
		{
			...
			`,
			validateSchema: func(s *Schema) error {
				typ, ok := s.Types["Product"].(*Interface)
				if !ok {
					return fmt.Errorf("type %q not found", "Product")
				}
				if len(typ.Fields) != 3 {
