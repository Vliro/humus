...
func writeHeader(w *bytes.Buffer) {
	_, _ = w.WriteString("package " + conf.Package + " \n")
...
func Parse(config *Config) {
	...
	*/
	...
	var hasFile = false
	var customFields map[string]map[string]Customs
		...
		if !info.IsDir() {
			...
			if fp := filepath.Ext(info.Name()); fp == ".toml" {
				...
				if info.Name() == "custom.toml" {
					customFields = parseCustoms(bytes.NewReader(fd))
					return nil
				}
			}
			hasFile = true
...
func createModel(s *schema.Schema, output io.Writer, enumWriter io.Writer) (error, map[string][]Field) {
	//We now have the schema relevant for the file. First generate the gen.
	obj := s.Objects()
...
func writeCustoms(customs map[string]map[string]Customs, output io.Writer) {
	var sb bytes.Buffer
